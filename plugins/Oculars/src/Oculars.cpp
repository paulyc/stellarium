...
void Oculars::enableOcular(bool enableOcularMode)
{
	...
	if (!flagShowOculars && !flagShowTelrad && flagRequireSelection && !StelApp::getInstance().getStelObjectMgr().getWasSelected() )
	{
		if (usageMessageLabelID == -1)
		{
			...
			int xPosition = qRound(projectorParams.viewportCenter[0] - 0.5 * metrics.horizontalAdvance(labelText));
			...
			if (width > 0.0f && height > 0.0f)
			{
				...
				if (ccdXRatio>=ratioLimit || ccdYRatio>=ratioLimit)
				{
					...
					QString scales = QString("%1%3 %4 %2%3")
							...
							.arg(unit)
							.arg(QChar(0x00D7));
					a = transform.map(QPoint(qRound(width*0.5f - painter.getFontMetrics().horizontalAdvance(scales)*params.devicePixelsPerPixel), qRound(-height*0.5f - fontSize*scaleFactor)));
					...
					a = transform.map(QPoint(qRound(width*0.5f - painter.getFontMetrics().horizontalAdvance(angle)*params.devicePixelsPerPixel), qRound(height*0.5f + 5.f)));
					...
					if(flagShowCcdCropOverlay && (ccdXRatio>=ratioLimitCrop || ccdYRatio>=ratioLimitCrop))
					{
						...
						QString resolutionOverlayText = QString("%1%2 %3 %1%2")
								...
								.arg(qc_("px", "pixel"))
								.arg(QChar(0x00D7));
						a = transform.map(QPoint(qRound(overlayWidth*0.5f - painter.getFontMetrics().horizontalAdvance(resolutionOverlayText)*params.devicePixelsPerPixel), qRound(-overlayHeight*0.5f - fontSize*scaleFactor)));
...
void Oculars::paintText(const StelCore* core)
{
	...
	const double insetFromRHS = painter.getFontMetrics().horizontalAdvance(widthString);
	StelProjector::StelProjectorParams projectorParams = core->getCurrentStelProjectorParams();
	int yPositionOffset = qRound(projectorParams.viewportXywh[3]*projectorParams.viewportCenterOffset[1]);
